...
```{r}
phenotype_class <- c("clinical_traits", "liver_genes", "liver_isoforms",
                     "liver_lipids", "liver_psi", "liver_splice_juncs",
                     "plasma_metabolites", "plasma_metabolites", 
                     "plasma_metabolites_13C", "plasma_metabolites_2H")
```

```{r}
peaks <- qtl2shiny::read_peak_class(project_info, class_name)
```
...
```
```{r}
pheno_metab <- qtl2shiny:::read_project(project_info,
                                       paste0("pheno_", class_name))
```

```{r}
(pheno_names <- names(pheno_metab)[grep("AUC_ARG", names(pheno_metab))])
```
...
```{r}
phe_mx <- pheno_metab[,grep(pheno_name, colnames(pheno_metab)), drop = FALSE]
rownames(phe_mx) <- pheno_metab$Mouse
...
```{r}
arg_mx <- pheno_metab[,match(pheno_names, colnames(pheno_metab)), drop = FALSE]
rownames(arg_mx) <- pheno_metab$Mouse
```
...
comediator_region <- function(pheno_name, chr_id, scan_window, 
  ...
  pheno_metab_local <- pheno_metab[, phenos, drop = FALSE]

  # Create comediator object.
  out <- pheno_region(chr_id, scan_window, covar, pmap,
    peaks, pheno_metab_local, drivers = qtls)
...
comed_ls <- comediator_region(pheno_name, chr_id, scan_window,
  covar, peaks, qtls, pmap_obj, pheno_metab)
